...
For this section you will write the code to create an object `buffer_iucn`. Use `st_buffer` to extend the IUCN polygon by choosing a distance (e.g. `distance = 50000` will be a distance of 50km). Use `leaflet` to plot three different buffer sizes of your choice. Your favourite of these should be named `buffer_iucn` and will be used later on.

Ask yourself:

-   Which buffer removes obvious errors?

-   Which keeps plausible edge populations?

-   At what point does the buffer become biologically unrealistic?

##### ⚠️ Important Point 1

Buffers measured in longitude/latitude can behave strangely because degrees are not equal distances across the globe. So first, we project the data into a coordinate reference system (CRS) measured in metres. Once you have created the buffers you may need to transform them back into a CRS that is measured in degrees of latitude and longitude (use `crs=4326`)
...
Now visualise everything together: occurrences, buffers, IUCN range, convex hull, and alpha hull, based on what you've learned using `leaflet`.
