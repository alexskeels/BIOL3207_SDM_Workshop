...
Your working directory should be your `DS4B_Week_7/` folder, with a `data/` subfolder inside it:
...
1.  **From Canvas** — download `data.zip` from the Week 7 module, unzip it, and move the contents into your `data/` folder.
2.  **From GitHub** — clone or download the `data/` folder directly from <https://github.com/alexskeels/BIOL3207_SDM_Workshop> and drop it into your `DS4B_Week_7/` folder.

**Check it worked:** run `list.files("data")` in the console. If the data files are listed, you're ready to go.

## Packages for today

Before you start Task 1, run this chunk **once** to install every package you'll need this week. `hull2spatial` is only on GitHub, so it needs `remotes` to install.
...
::: callout-note
The code chunks in *this* instructions document have `eval: false` set globally, which is why no output appears if you knit this file. You should be writing your code in your **own** `workshop_solutions` `.qmd` (created in step 7 of "Before you begin"), which should have `eval: true`.
:::
...
Save the missingness tibble to a variable (e.g. `missingness_table`) and use it as input to `ggplot()`. Map `n_missing` to the x-axis and use `geom_histogram()`. If you get stuck, click "Show solution" in the code block below.
:::
...
#### **Task 2. Explore the spatial data**
...
#### **Task 3. Compare expert range vs occurrence records**
...
#### **Task 4. Explore buffer distances**
...
Now create at least two more buffers of different sizes (e.g. 100 km, 200 km, 500 km) using the same steps. Plot all three on a `leaflet` map together with the occurrence points and the IUCN polygon, using different colours for each buffer. Here is a template for layering multiple polygons on one map:

```{r}
# Template: multi-layer leaflet map with different colours per polygon
leaflet(occ_sf) |>
  addProviderTiles(providers$OpenStreetMap.Mapnik) |>

  # Occurrence points
  addCircleMarkers(radius = 3, fillOpacity = 0.5, stroke = FALSE, color = "#2C7FB8") |>

  # IUCN polygon (green)
  addPolygons(data = iucn_sf, fillColor = "#009E73", color = "#007F5F",
              weight = 2, fillOpacity = 0.2, popup = "IUCN Range") |>

  # Buffer 1 (gold) — replace buffer_50km with your object name
  addPolygons(data = buffer_50km, fillColor = "#FFD700", color = "#B8860B",
              weight = 1, fillOpacity = 0.1, popup = "50 km buffer") |>

  # Buffer 2 (orange) — REPLACE `your_second_buffer` with the buffer object you made above
  addPolygons(data = your_second_buffer, fillColor = "#FFA500", color = "#FF8C00",
              weight = 1, fillOpacity = 0.1, popup = "Your second buffer")
```

Assign your preferred buffer to the variable `buffer_iucn`. **Make sure you do this before moving on** — it is used in the next task.

```{r}
# YOUR CODE HERE: replace the right-hand side with your favourite buffer object
# (e.g. buffer_iucn <- buffer_100km). The default below uses the 50 km buffer
# so Task 5 still runs if you skip this step — but do make a choice of your own.
buffer_iucn <- buffer_50km
```
...
1.  Make sure `alphahull`, `hull2spatial` and `sp` are loaded (they were listed in the "Packages for today" chunk at the top of this workshop — if you skipped that, go install and load them now).
...
terra::writeVector(terra::vect(ahull_sf), "data/frilled_lizard_alpha_hull.shp", overwrite = TRUE)
```
...
-   You have saved your cleaned occurrence data as a CSV (`data/frilled_lizard_ALA_cropped.csv`)
-   You have saved your alpha hull as a shapefile (`data/frilled_lizard_alpha_hull.shp`)
-   You have written reflections on the "Ask yourself" questions in your `.rmd`
